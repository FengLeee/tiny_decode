...
uint16 WaveData::read_uint16(FILE* f , int32 swap)
{
	uint16 uinteger16 ;
...
uint32 WaveData::read_uint32(FILE* f , int32 swap)
{
	uint32 uinteger32 ;
...
int WaveData::read_quad_bytes(FILE* f , char* quad_bytes)
{
...
int WaveData::expect_quad_bytes(FILE* f , const char* expected_quad_bytes)
{
	char read_bytes[5] ;
	read_bytes[4] = '\0' ;
 ...
 */
int WaveData::read(const char* wave , BaseFloat** data , uint32* length)
{
