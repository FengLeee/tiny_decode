...
static const double64 var_floor = 1e-20 ;
...
void CMN::apply_cmn(BaseFloat* input_feature , const uint32 num_row , const uint32 num_col)
{
	...
	if(NULL == norm_)
	{
		double64* sum = (double64*)malloc(dim_ * sizeof(double64)) ;
		norm_ = (double64*)malloc(dim_ * sizeof(double64)) ;
		if(NULL == sum || NULL == norm_)  error("no memory to be allocated" , __FILE__ , __LINE__) ;

		memset((void*)sum , 0 , dim_ * sizeof(double64)) ;

		for(uint32 r = 0 ; r < window_size_ ; r++)
			for(uint32 c = 0 ; c < dim_ ; c++)  sum[c] += (double64)stats_[r * dim_ + c] ;
...
void CMN::local_normalize(BaseFloat* input_feature , const uint32 num_row , const uint32 num_col)
{
	...
	double64* sum = (double64*)malloc(dim_ * sizeof(double64)) ;
	double64* norm = (double64*)malloc(dim_ * sizeof(double64)) ;
...
void CMVN::apply_cmvn(BaseFloat* input_feature , const uint32 num_row , const uint32 num_col)
{
	...
	{
		double64* sum = (double64*)malloc(dim_ * sizeof(double64)) ;
		double64* squared_sum = (double64*)malloc(dim_ * sizeof(double64)) ;
		norm_ = (double64*)malloc(dim_ * sizeof(double64)) ;
		squared_mean_ = (double64*)malloc(dim_ * sizeof(double64)) ;
		var_ = (double64*)malloc(dim_ * sizeof(double64)) ;
		if(NULL == sum || NULL == squared_sum || NULL == norm_ || squared_mean_ == NULL || var_ == NULL)  error("no memory to be allocated" , __FILE__ , __LINE__) ;

		memset((void*)sum , 0 , dim_ * sizeof(double64)) ;
		memset((void*)squared_sum , 0 , dim_ * sizeof(double64)) ;

		for(uint32 r = 0 ; r < window_size_ ; r++)
			for(uint32 c = 0 ; c < dim_ ; c++)
			{
				sum[c] += (double64)stats_[r * dim_ + c] ;
...
void CMVN::local_normalize(BaseFloat* input_feature , const uint32 num_row , const uint32 num_col)
{
	...
	double64* squared_sum = (double64*)malloc(dim_ * sizeof(double64)) ;
	double64* norm = (double64*)malloc(dim_ * sizeof(double64)) ;
	double64* squared_mean = (double64*)malloc(dim_ * sizeof(double64)) ;
	double64* var = (double64*)malloc(dim_ * sizeof(double64)) ;
