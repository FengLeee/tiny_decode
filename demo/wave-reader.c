...
static void error(const char* error_message , const char* file , uint32 line)
{
...
int wav_read(const char* wave , BaseFloat** data , uint32* length)
{
	...
	if(subchunk1_size < 16)  error("expect PCM format data to have fmt chunk of size of at least 16" , __FILE__ , __LINE__)  ;
		...
		uint32 i ;
		for( i = 16 ; i < subchunk1_size ; ++i)   fgetc(f) ;
	}
...
/*	uint32 remainder = data_chunk_size % block_align_;
	if(0 != remainder)
	{
		char err[100] ;
		sprintf(err , "data chunk size has unexpected length %u , block align is %u" , data_chunk_size , block_align_) ;
		error(err , __FILE__ , __LINE__) ;
	}
*/
	...
	for(i = 0 ; i < num_samples ; i++)
	{
		for( j = 0 ; j < num_channels ; j++)
		{
			switch(bits_per_sample)
			{
				...
				default :
					error("not supported bits per sample" , __FILE__ , __LINE__) ;    
			}
		}
	}
